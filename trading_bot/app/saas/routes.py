...
def admin_backtest_start(request: Request, admin: dict = Depends(require_admin)) -> dict:
    ...
    try:
        ...
        if df is None or len(df) < 100:
            return {"ok": False, "error": "server can't pull Bybit candle history "
                    f"(got {0 if df is None else len(df)} bars) — likely a region/IP block."}
        ...
        threading.Thread(target=_run, args=(request.app.state, _BT_SYMBOLS, _BT_WINDOWS),
                         daemon=True).start()
        return {"ok": True, "status": "running", "message": "backtest started (v2)"}
    except Exception as exc:  # never a blank 500 — return the reason in the body
        return {"ok": False, "error": f"{exc} | {traceback.format_exc()[-300:]}"}
